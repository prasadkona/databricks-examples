"""
Utility functions for the SEC Financial Analyst Multi-Agent.
"""
...
def get_session_id(request: ResponsesAgentRequest) -> str | None:
    """Extract session ID from request for conversation tracking."""
...
def build_mcp_url(path: str, workspace_client: WorkspaceClient | None = None) -> str:
    """Build full MCP URL from relative path."""
...
def get_user_workspace_client() -> WorkspaceClient:
    """Get a workspace client authenticated as the requesting user."""
...
async def process_agent_stream_events(
    ...
    """Process streaming events from the agent runner."""
